...
arqs=os.listdir("./corpus")
arqs=[a for a in arqs if a[-3:]=="krn"]
arqs.sort()

for a in arqs:
    print "===========\n"+a
    # importamos Piano Sonata No. 1 in F  minor, Op. 2, No. 1 [1793-5]
    opus = m.converter.parse('corpus/%s' % (a,))

    # vozes
    vozes=[voz.id for voz in opus.parts]
    # spine_2 => G
    # spine_0 => F
    for v in vozes:
        print v
        ...
        #print 'Encontrados %s motivos iguais a %s.' % (encontrados[i], intervalos_scar[i])
        ...
        for o in onde:
            ...
            print len(compassos_incidencias), compassos_incidencias[-1]
            print "\n"
